...
DEFAULT_IP_LISTEN = "0.0.0.0"

# ===== PORT ENV =====
DEFAULT_SOCKET_PORT = 8888
socket_port = os.environ.get("CHAT_PORT", DEFAULT_SOCKET_PORT)
try:
    socket_port = int(socket_port)
except:
    print(f"CHAT_PORT bad value, default port : {DEFAULT_SOCKET_PORT}")
    socket_port = DEFAULT_SOCKET_PORT

print("Port set: "+str(socket_port))

# ===== MAX USERS ENV =====
DEFAULT_SERVER_MAX_USERS = 3
server_max_users = os.environ.get("MAX_USERS", DEFAULT_SERVER_MAX_USERS)
try:
    server_max_users = int(server_max_users)
except:
    print(f"MAX_USERS bad value, default max users : {DEFAULT_SERVER_MAX_USERS}")
    server_max_users = DEFAULT_SERVER_MAX_USERS

print("Max users set: "+str(server_max_users))
...
MSG_MAX_USERS = "ʕ°ᴥ°ʔ ( Too much users connected ! )"
...
async def handle_client_msg(websocket, redis_client, clients_websocket):
    ...
    if len(clients_websocket.keys()) >= server_max_users:
        await send_to_client(websocket, MSG_MAX_USERS)
        websocket.close()
        return
...
async def main():
    ...
    # Redis connection
    redis_client = redis.Redis(host="chat_redis", port=6379, decode_responses=True)
    try:
    ...
    async with websockets.serve(lambda ws: handle_client_msg(ws, redis_client, clients_websocket), DEFAULT_IP_LISTEN, socket_port):
        print(f"Server listen on ws://{DEFAULT_IP_LISTEN}:{str(socket_port)}")
